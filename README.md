...
The application is built on a modern three-tier client-server architecture:
 ...
 └──────────────────────┘        └────────────────────────┘
Frontend (Presentation): React (v19) compiled with Vite. Uses Tailwind CSS for responsive layouts and Axios for asynchronous API client calls.
Backend (Application): Node.js and Express. Exposes REST endpoints, validates inputs, manages JWT login sessions, records administrative audit logs, and coordinates Gemini calls.
Database (Persistence): MongoDB using Mongoose schemas to map Patient, Review, Admin, Warning Alert, and Audit Log collections.
🛠️ Technology Stack
